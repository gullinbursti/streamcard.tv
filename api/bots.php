...
if ($_POST['action'] == "INSERT") {
	$query = 'INSERT IGNORE INTO `bots` (`id`, `company_id`, `name`, `info`, `app_id`, `logo_url`, `added`) VALUES (NULL,  "'. $_POST['name'] .'", "'. $_POST['display_name'] .'", "'. $_POST['email'] .'", "'. $_POST['password'] .'", "'. $_POST['logo_url'] .'", NOW());';
	$result = mysqli_query($db_conn, $query);

} else if ($_POST['action'] == "UPDATE") {
	$query = 'UPDATE `bots` SET `name` = '. $_POST['name'] .', `info` = '. $_POST['info'] .', `app_id` = '. $_POST['app_id'] .', `logo_url` = '. $_POST['logo_url'] .' WHERE `id` = '. $_POST['id'] .';';
	$result = mysqli_query($db_conn, $query);
