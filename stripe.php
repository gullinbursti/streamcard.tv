...
$query = 'SELECT `name`, `info`, `price`, `image_url` FROM `fb_products` WHERE `id` = '. $_GET['item_id'] .' LIMIT 1;';
...
if (mysql_num_rows($result)) {
	...
	$item_image = str_replace("http://", "https://", $item_obj->image_url);

} else {
  header("Location: http://prebot.chat/");
}
...
curl_setopt_array($curl, array(
	...
	CURLOPT_POSTFIELDS => http_build_query(array(
		...
		'cid' => md5($_SERVER['REMOTE_ADDR']),
		't'   => "event",
		'ec'  => "buy-page-load-". ((intval($_GET['from_user']) > 0) ? "FACEBOOK" : "KIK"),
		'ea'  => "",
		'el'  => "",
		'ev'  => "0"
	)),
...
//		\Stripe\Stripe::setApiKey("sk_test_3QTHaM9IjN2g3AI91Gqqaoxo");
	...
	$query = 'SELECT `chat_id` FROM `kikbot_logs` WHERE `username` = "'. $_GET['from_user'] .'" LIMIT 1;';
	$result = mysql_query($query);
...
<html>
    ...
    <div class="price-area" style="width:100%">
			...
			<img src="img/gamebots-intro.gif" width="100%"><br>
